...
- Never write that something is attached without verifying it: `favro get` and
  `attachments[].name` is the only proof. Since 0.2.1 the CLI preserves attachments across
  description writes and fails loudly if it cannot, so the old "attach last" workaround is gone —
  but a claim written before the file was uploaded, or a file that was never uploaded at all, is
  still just a claim. The same applies in reverse: a file that exists only as a card attachment is
  not in the repository, so cite both when both matter.
